...
class RetornosTemporais():
    ...
    def calculos_indicadores(self, weights, retornos_log):
        ...
        self.retorno = np.sum(self.weights*self.retornos_log.mean())*252
        self.variancia = np.dot(self.weights.T, np.dot(self.retornos_log.cov() * 252, self.weights))
        self.volatilidade = np.sqrt(np.dot(self.weights.T, np.dot(self.retornos_log.cov() * 252, self.weights)))
        self.sharpe = (self.retorno - 0.1058) / self.volatilidade
...
class AlgoritmoBruto():
    ...
    def resolver(self):
        ...
        for x in range(self.numero_execucoes):
            ...
            pfolio_returns.append(np.sum(weights * self.retornos_log.mean()) * 252)
            pfolio_volatilities.append(np.sqrt(np.dot(weights.T, np.dot(self.retornos_log.cov() * 252, weights))))
            pfolio_sharpe.append((pfolio_returns[x] - 0.1058) / pfolio_volatilities[x])
...
class AlgoritmoGenetico():
    ...
    def resolver(self, taxa_mutacao):
        ...
        print("\n== Melhor solução == \n G: %s\n Sharpe: %s\n Cromossomo: %s\n Weights: %s\n Retorno Esperado: %s\n Volatilidade: %s" % (self.melhor_solucao.geracao, self.melhor_solucao.nota_avaliacao, self.melhor_solucao.cromossomo, self.melhor_solucao.weights, self.melhor_solucao.retorno, self.melhor_solucao.volatilidade))
        return self.melhor_solucao.cromossomo
                
class Database():
    
    def __init__(self):
        self.connect = pymysql.connect(host="localhost", user="root", passwd="", db="tcc")
...
if __name__ == '__main__':
    ...
    ativos = Ativos('2015-1-1', '2017-1-1', '495')
    
    frameAcoes = ativos.setFrame('2015-1-1', '2017-1-1',)
    ...
    retornos_log.mean() * 252 #retornos dos ativos anual
    (retornos_log.mean() * 252).tail()
    retornos_log.plot(figsize=(8,5))
    plt.show()
    ...
    ag = AlgoritmoGenetico(20, ativos.lista_ativos, dataframe1, model, 50, retornos_log)
    ...
    #cov_matrix
    count_total = 0 
    print(ag.pfolio_widgets[ag.pfolio_sharpe.argmax()])    
    for i in range(len(ativos.lista_ativos)):
        count_total += ag.pfolio_widgets[ag.pfolio_sharpe.argmax()][i]
        print("ATIVO: %s PESO: %s " % (ativos.lista_ativos[i].nome, ag.pfolio_widgets[ag.pfolio_sharpe.argmax()][i]))
